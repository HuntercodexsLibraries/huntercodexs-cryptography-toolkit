...
public class CryptographyToolkitProcessor {
    ...
    public static String getSecretFromPreDefinedSource(CryptographyContract contract) {
        ...
        throw new CryptographyException("Cryptography Toolkit Fail: Secret Spec (getSecretFromPreDefinedSource)");
    }

    public static String getIvFromPreDefinedSource(CryptographyContract contract, String data, boolean isDecrypt) {
        if (contract.getCryptographyIvSource() == IV_FROM_PARAMETER) {
            ...
            if (isDecrypt) {
                return dataGeneratorForDecryptStaticRobust(data);
            }
            return UUID.randomUUID().toString().toUpperCase().replaceAll("-", "").substring(0, 16);
        }

        throw new CryptographyException("Cryptography Toolkit Fail: IV Parameter Spec (getIvFromPreDefinedSource)");
    }
    ...
    public String getIvFromSource(String data, boolean isDecrypt, boolean isBasic) {
        if (this.contract.getCryptographyIvSource() == IV_FROM_PARAMETER) {
            ...
            if (isDecrypt && !isBasic) {
                return dataGeneratorForDecryptRobust(data);
            } else if (isDecrypt) {
                return data;
            }
    ...
    public static String dataGeneratorForEncryptStaticRobust(String ivSource, byte[] encryptedData) {
    ...
    public static String dataGeneratorForDecryptStaticRobust(String dataToDecrypt) {
    ...
    public static String dataGeneratorForEncryptAutomatic(String ivSource, String secretKey, byte[] encryptedData) {
        int size = Base64.getEncoder().encodeToString(encryptedData).length();
        int half = size / 2;
    ...
    public static String dataGeneratorForDecryptAutomatic(String dataToDecrypt) {
        String[] parts = dataToDecrypt.replaceAll(SECRET_CONCAT_VALUE, "=").split("=");
    ...
    public String dataGeneratorForEncryptRobust(String ivSource, byte[] encryptedData) {
        int size = Base64.getEncoder().encodeToString(encryptedData).length();
        int half = size / 2;
    ...
    public String dataGeneratorForDecryptRobust(String dataToDecrypt) {
